...
if (isset($_POST['product_name']) && isset($_POST['description']) && isset($_POST['prix']) && isset($_POST['quantite_stock'])) {
    ...
    if (move_uploaded_file($imageTmp, $uploadDir . $image)) {
        ...
        $query = "INSERT INTO produit (nom, description, prix, quantite_stock, Categorie_ID, image) VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $db->prepare($query);
        // Utiliser le type 'i' pour Categorie_ID qui est un entier
        $stmt->bind_param("sssiss", $product_name, $description, $prix, $quantite_stock, $category, $image);
